...
### 20. 🌐 公网 IP 分配策略审计与修复：宿主代答、别名回收与 ICMP 透传 (Public IPv4 Ownership - v1.20.12)
- **问题背景**：给 vm-3 分配独立公网 `23.95.253.12` 后"连不通"。实测发现是**两个真实缺陷**加上一个测试视角误导（详见下）。
- **缺陷①：宿主机在公网 IP 上代答所有端口**。CLICD 为每个已分配的独立 IPv4 在出口网卡上添加别名（`eno1:clicd`，这是让上游 ARP 能解析所必需），但没有对应的回收逻辑 —— 别名一旦加上就永久保留。宿主机既然拥有了该地址，**任何没有被 DNAT 转发的端口都会被内核交给本机服务**，于是：未分配的 `.13`（池内但无人使用）与已关机实例的 `.11` 上，宿主机的 sshd 与面板对公网开放；而用户会看到 IP "有响应"却连不上自己的服务。实测确认 `.11:22` 从外部可直接敲到宿主机 sshd。
  - **修复**：新增 `ReconcilePublicIPv4Aliases()`（只清理带 `:clicd` 标签、且已不属于任何实例的别名）与 `EnsurePublicIPv4LocalDeliveryGuard()`（对池内 + 已分配的全部公网地址加 `INPUT -d <ip> -j DROP`）。DNAT 后的流量被路由到网桥、不经过 INPUT，因此守卫只消灭"假应答"，碰不到正常转发与从内网发起的 SNAT 回程。
- **缺陷②：ICMP 不走 DNAT，ping 由宿主机代答**。全端口透传只覆盖 tcp/udp，`ping <公网IP>` 一直是宿主机内核在回复 —— 客户机没起来、或客户机防火墙挡了 ICMP，ping 依旧"通"。**修复**：透传规则增加 `-p icmp --icmp-type echo-request -j DNAT`，ping 从此反映客户机的真实状态。
- **数据修正**：vm-3 的 SSH 映射原为 `22002 → 22`，但该客户机（PVE 迁移过来的麒麟系统）sshd 实际监听 **2222**，映射指向空端口。已改为 `22002 → 2222`。
- **测试视角提示（非缺陷）**：从**同一个 NAT 网段内**的机器（例如在内网 VM 里）访问自己的公网 IP 属于 hairpin 场景，必然失败 —— 客户机发现目标同网段会直接二层回包、绕过宿主机，发起方等不到"来自公网 IP 的回复"。判断公网可达性必须在外部网络验证。
- **涉及文件**：`backend/internal/lxc/{ipv6,portmap}.go`、`backend/internal/lxc/ipv6_guard_test.go`、`backend/internal/kvm/kvm.go`（分配变更后同步回收/守卫）。
- **实机验证**：以跨网段的 LXC 容器为测试客户端（源 `10.0.3.x`，回包必须经宿主机，规避 hairpin）：`.12:2222` 可连接、`.13:22`/`.13:80`/`.11:22` 全部连不上（修复前这些端口由宿主机应答）、内网直连对照正常；宿主机自身 `ping` 公网 IP 由"能通"变为 100% 丢包；客户机出网 SNAT 会话保持 ESTABLISHED（未被守卫误伤）；独立外部节点（伊朗/葡萄牙/斯洛文尼亚/土耳其/西班牙）连接 `.12:2222` 全部成功。
- **缺陷③：全端口透传抢先匹配，自定义端口映射被吃掉**。所有规则都以 `-I PREROUTING 1` 插入，因此**后插入者在上**；透传规则原本在端口映射之后插入，于是永远压在映射之上 —— 连接 `公网IP:22002` 会先被透传改写成 `客户机:22002`，`22002 → 2222` 这条映射根本没机会生效（客户机 22002 无监听 → 连不上）。**修复**：把透传规则的插入提到端口映射之前，使其落在映射之下，自定义映射优先。
- **缺陷④（自引入并当日修复）**：守卫规则的清理逻辑取注释时把整行剩余部分一起吞掉（`clicd-pubguard-… -j DROP`），导致"保留集合"永远匹配不上、**刚插入的守卫在下一次清理时被自己删除**（线上实测启动后守卫数归零）。**修复**：按空白截断只取注释 token 并容忍引号形式，保留集合直接比较生成的注释串；补单元测试 `ipv6_guard_test.go` 覆盖两种注释格式与普通规则行。
...
### 21. 🔗 独立公网 IP 互连修复：发夹 NAT、宿主机反射与守卫收窄 (Hairpin NAT - v1.20.13)
- **问题**：给实例分配独立公网 IP 后，**同一网络内的机器（含宿主机自己）用公网 IP 访问另一个实例会失败**；用户指出"以前用 PVE 时独立 IP 之间是可以互相连通的"。
- **根因（架构差异，不是偶发故障）**：PVE 模式下公网 IP 是**配置在客户机内部**的，虚拟机之间走二层直连、回包源地址就是公网 IP，因此天然互通；而 CLICD 采用**公网 IP 建在宿主机 + DNAT 转发**的模式：客户机 A 发往 `公网IP` 的包被宿主机改写目标为另一台客户机 B，但 B 回包时发现 A 与自己**同网段**，直接二层回给 A、**绕过了宿主机** —— A 收到的是"来自内网地址的回复"，与它拨号的公网 IP 不匹配而丢弃，表现为超时（实测 conntrack 中该连接长期停在 `[UNREPLIED]`）。
- **修复①（发夹 NAT）**：新增 `EnsurePublicIPv4HairpinMasquerade()`，对「源与目标同属一个容器网桥子网、且该连接已被 DNAT」的流量做 MASQUERADE（`-m conntrack --ctstate DNAT`），强制回包经过宿主机。**只影响经公网 IP 绕回的连接，客户机之间的直连流量不受影响**；对 KVM（192.168.122.0/24）与 LXC（10.0.3.0/24）各一条，随服务启动与分配变更重新下发。
- **修复②（宿主机反射）**：宿主机自己访问这些公网地址时，包在 OUTPUT 链就被判定为本机地址而投递给自己。新增 nat **OUTPUT** 反射规则（透传 + 逐条端口映射各一份，注释带 `-out` 后缀，随 `CleanPortMappings` 一起清理），使宿主机也能用公网 IP 访问实例。
- **修复③（守卫收窄）**：上一版为防止宿主机在公网 IP 上代答而加的 `INPUT -d <ip> -j DROP` 过宽，会把宿主机自己发起的反射连接的回包一并丢掉（实测 conntrack 已是 `SYN_RECV`、客户机已回 SYN-ACK 但宿主机收不到）。现收窄为 **`-m conntrack --ctstate NEW`**：只拦"新建入站连接"，已建立会话的回包放行 —— 防护效果不变（未分配/已关机 IP 仍无响应），宿主机自测也通了。
- **涉及文件**：`backend/internal/lxc/{portmap,ipv6}.go`。
- **实机验证**：
  - **用户场景**：从同网段的 vm-4 连接 `23.95.253.12:2222` 与 `:22002`，均收到客户机 sshd 横幅 `SSH-2.0-OpenSSH_10.1`；
  - 宿主机连接 `23.95.253.12:2222` 可连接（反射生效），`:22` 连不上（客户机未监听，符合预期）；
