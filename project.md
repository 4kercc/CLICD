...
本文档面向后续 AI 接力开发与架构维护，全面汇总了 **CLICD (LXC/KVM 虚拟化管理面板)** 的系统架构、各模块代码职责、核心技术设计、近期的关键改动与演进记录（涵盖 v1.20 ~ v1.20.9，含 Telegram Bot、预设初始化命令、Windows 装机链路、额外挂载光盘、快照备注与快照占用统计等），并附带现存待办需求与运维指令。
...
### 3. 🤖 原生内置 Telegram Bot 模块 (v1.20.4)
...
### 6. 🛡️ LXC 硬件 MAC 固化与 IP 防漂移 (v1.20.5)
...
### 7. 🌐 NAT 子网过滤与防 Docker 网桥干扰 (v1.20.5)
...
### 9. 🚀 容器/虚拟机预设初始化命令 (Init Script) (v1.20.7)
...
### 11. 💿 KVM Windows ISO 挂载防呆与 `__invalid_image_id__` 启动报错修复 (v1.20.7)
- **根因分析**：
...
### 12. ⚡ Usage 资源指标 5 秒硬超时与列表渲染解耦 (v1.20.7)
...
### 13. 🪟 创建向导「选 Windows 却装出 Debian 12」根因修复 (v1.20.8)
...
### 14. ⏱️ 「一直卡在关机中 / 无法删除」任务假死修复 (v1.20.8)
...
### 16. 🧩 第三方 WinPE / WePE 镜像无法添加修复 (v1.20.8)
...
### 17. 🌐 英文界面残留中文清理与 i18n 覆盖补全 (v1.20.8)
...
### 18. 💿 额外挂载光盘（独立光驱）+ 服务器镜像选择器 (v1.20.8)
- **需求背景**：
