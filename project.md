...
### 2. 📊 快照占用拆分为「逻辑占用 / 实际独占」(v1.20.9)
- **问题背景**：快照列表只显示 `du` 口径体积，用户看到 `jsq-windows` 一份快照显示 29.3 GB 后质疑"真实占用不该这么大"。排查确认：宿主 XFS（`reflink=1`）下 `cp --sparse=always` 实际执行 **reflink 克隆**（实测克隆 29 GiB 仅 2.8 秒、`df` 只涨约 4 MB），快照与运行盘共享物理块，而 `du` 会把共享块在每个文件里各算一遍——因此该数字是**体积上界**，不是当下物理占用。
- **实现**：新增 `backend/internal/api/snapshot_usage_linux.go`。`x/sys/unix` 未封装 FIEMAP，故手写 `FS_IOC_FIEMAP`（`0xC020660B`）ioctl 遍历 extent，累加带 `FIEMAP_EXTENT_SHARED`（`0x2000`）标志的长度；**实际独占 = SizeBytes − 共享字节 = 删除该快照能真正释放的空间**。带 20 秒 TTL 内存缓存（共享块会随 guest 写入不断转为独占，结果本身在变），缓存上限 256 条。非 Linux 与 LXC（rootfs 目录树、无单个可克隆镜像）返回 `nil`，由 `snapshot_usage_fallback.go` 及前端 `-` 兜底。
- **接口与字段**：`config.Snapshot` 新增 `UniqueBytes *int64`（`json:"unique_bytes,omitempty"`），**只读计算、不落库**；`HandleSnapshots`、`listContainerSnapshots`、`createContainerSnapshot`、`syncContainerSnapshot` 四处统一经 decorator 填充。
- **前端**：容器详情 `SnapshotTable` 与 `Snapshots.tsx` 全局列表把「大小 / 占用空间」拆为「逻辑占用」与「实际独占」两列（独占绿色标注、不可测显示 `-`），表头 `title` 说明 reflink 共享与"随写入逐步变大"；`TableHead` 增加 `title` 支持；全局表 `overflow-hidden` → `overflow-x-auto` 并加宽到 `min-w-[1060px]`。
- **涉及文件**：`backend/internal/api/{snapshot_usage_linux,snapshot_usage_fallback,snapshots}.go`、`backend/internal/config/config.go`、`frontend/src/pages/{ContainerDetail,Snapshots}.tsx`、`frontend/src/services/api.ts`、`frontend/src/utils/i18n.ts`。
- **实机验证（测试机）**：接口 `unique_bytes` 与 `filefrag` 独立统计一致——`jsq-windows` 最新快照逻辑 29.26 GiB / 独占 0.18→0.24 GiB（随写入增长）、09-20 那份 24.47 / 11.37 GiB、`kylin-v10` 0.21 / 0.00 GiB；全局列表首次 1.02s、缓存后 0.05~0.08s；LXC 快照正确返回 `null` 并由前端显示 `-`。三份 `jsq-windows` 快照逻辑合计 61.55 GiB 而真正独占仅 18.14 GiB，印证 `du` 为体积上界。
- **维护提示**：该特性依赖宿主文件系统。ext4 或无 reflink 的 XFS 上同一份代码会退化为真实全量复制（几十 GB、耗时数分钟），届时「实际独占」自然等于「逻辑占用」，面板无需改动即自洽。

### 3. 🛠️ 三个稳定性缺陷修复：还原权限 / 镜像删除护栏 / VirtIO 光盘 (v1.20.9)
- **① 还原快照或备份后 QEMU 无法读盘**（`Cannot access storage file ... (as uid:64055): Permission denied`）
  - 根因：快照/备份目录以 `0700 root:root` 创建，`RestoreSnapshot`、`RestoreBackup` 用 `copyTree` 把它们原样复制回 `instances/vm-X`，非 root 的 QEMU 进程（libvirt-qemu, uid 64055）无法进入该目录。
  - 修复：新增 `kvmQEMUIdentity()`（探测 `libvirt-qemu`/`qemu` 及其主组）与 `fixKVMInstancePermissions()`，调用点为 **`StartContainer` 每次开机前**、`RestoreSnapshot` 与 `RestoreBackup` 的 `copyTree` 之后。目录维持 `0700`（不把租户磁盘暴露给其它本地账号），仅把属主交给 QEMU 用户；探测不到 QEMU 用户时退回 `0755/0644` 以确保仍可启动。**注意：同一修复曾以 `215a65f` 提交过，但在 main 分支历史重置中丢失，本次连同回归测试一起重新落地——改这块代码前先确认它在位。**
- **② 删除镜像使实例永久无法启动**（`Cannot access backing file ...: No such file or directory`）
  - 根因：删除镜像仅校验 `container.Template == imageID`，而 `Template` 是运维可改写的展示标签（例如 vm-3 标签为 `kylin`、vm-25 为 `windows`），因此基于 `custom-kvm-9a78b2756f` 的 vm-3 在母盘被删后失去 backing file，全盘无副本、数据不可恢复。
  - 修复：新增 `kvm.InstancesUsingImage(id)` —— 用 `qemu-img info -U --backing-chain --output=json` 读取真实依赖链（`-U` 才能读运行中实例），**按文件名主干（stem）匹配**而非路径（`ImagePath` 对 windows 镜像会拼成 `.iso`，按路径比对必定失配）；`qemu-img` 因母盘缺失打不开时，回退 `qcow2BackingPath()`/`qcow2ChainFromHeaders()` 直接解析 qcow2 头。`handleCustomKVMImageDelete`、`HandleImageDelete` 两处删除前经 `imageInUseMessage()` 拦截并指名占用实例。
- **③ VirtIO 驱动光盘实际是 4KB 网页**（Guest Agent / 驱动装不上的真正原因）
  - 根因：镜像源 `fedorapeople.org` 对非浏览器请求返回 Anubis 反机器人 HTML 且状态码为 **200**；`ensureVirtioWinISO` 只判断 `os.Stat` 存在即复用，导致 4473 字节的 HTML 长期被当作 `virtio-win.iso` 挂载。
  - 修复：`virtioWinISOUsable()` 校验（≥100MB、拒绝 HTML 前缀、校验 ISO9660 `0x8001` 处 `CD001` 魔数），缓存无效即删除重下；下载经 `downloadFileWithValidator` 并校验临时文件后原子改名；`CLICD_VIRTIO_WIN_ISO_URL` 可覆盖源列表（逗号/空格分隔，支持 `http(s)://` 或本机路径，systemd 已通过 `/etc/clicd/network.env` 透传环境变量）。
...
- **运维铁律**：迁移进来的母盘（`custom-kvm-9a78b2756f`、`custom-kvm-f58ab36672` 等，后者 URL 为 `https://example.com/...` 占位）**绝不能删**，删掉无法重新下载；`/home/cvc/vm-101-disk-0.qcow2`(60 GiB)、`/home/cvc/vm-301-disk-0.qcow2`(100 GiB) 是它们的原始迁移源盘，属最后一道保险，禁止清理。判断依赖一律用 backing chain（见第 3 节②），不要相信 `Template` 标签或注册 URL。
- **实机验证**：恢复后 vm-3 控制台截屏为麒麟桌面登录界面（用户 `cvc`/`dmdba`/`user`、系统时间同步，原数据完好）；`net-dhcp-leases` 获得 192.168.122.251、ARP REACHABLE、`domifstat` 收发非零、宿主机 ping 0% 丢包；另在 00:09:53 的一次**真实快照还原**后确认 vm-25 目录归属为 `libvirt-qemu:kvm`（第 3 节①的修复在生产路径上得到验证）。`go vet` 与 `go test ./...` 全绿。

### 5. 🌐 公网 IP 分配策略审计与修复：宿主代答、别名回收与 ICMP 透传 (v1.20.12)
- **触发**：给 vm-3 分配独立公网 `23.95.253.12` 后「连不通」。审计后确认两个真实缺陷（外加一个测试视角误导）。
- **缺陷① 宿主机在公网 IP 上代答（安全 + 误导）**：`EnsureAssignedPublicIPv4s()` 为每个已分配 IPv4 在出口网卡加别名（`eno1:clicd`，上游 ARP 必需），但**没有回收逻辑**——别名一旦加上永久保留。宿主机拥有了该地址，任何未被 DNAT 转发的端口就被交给本机服务，于是未分配的 `.13`、已关机实例的 `.11` 上，宿主机 sshd/面板对公网开放（实测 `.11:22` 从外部直连到宿主机 sshd），用户看到「IP 有响应却连不上自己的服务」。
  - 修复：新增 `ReconcilePublicIPv4Aliases()`（仅清理带 `:clicd` 标签且不再属于任何实例的别名，启动时与每次分配变更后执行）与 `EnsurePublicIPv4LocalDeliveryGuard()`（对「池内 + 已分配」全部公网地址加 `INPUT -d <ip> -j DROP`，注释统一为 `clicd-pubguard-<ipTag>`，配置变更时自动清理过期规则）。DNAT 后的流量被路由到网桥、不经过 INPUT，故守卫只消除假应答，不影响转发与 SNAT 回程（已实测客户机出网会话保持 ESTABLISHED）。
- **缺陷② ICMP 不进 DNAT，ping 由宿主机代答**：全端口透传只覆盖 tcp/udp，`ping 公网IP` 一直是宿主机内核回复——客户机没起来、或客户机防火墙挡了 ICMP，ping 依旧「通」。修复：在 `ensureIndependentIPv4Ingress()` 增加 `-p icmp --icmp-type echo-request -j DNAT --to-destination <guest>`（注释同前缀，随 `CleanPortMappings` 一起清理）。
- **数据修正**：vm-3 的 SSH 映射由 `22002 → 22` 改为 `22002 → 2222`（该麒麟系统 sshd 实际监听 2222，原映射指向空端口）。
- **测试视角铁律**：**内网机器访问自家公网 IP 属 hairpin，必然失败**——客户机发现目标同网段会直接二层回包、绕过宿主机，发起方等不到来源为公网 IP 的回复。验证公网可达性必须用外部节点；本次诊断中我一度误判，正因为所在的 Windows 主机本身就是 vm-4（其出口公网 IP 恰为 `.10`）。若要让内网也能用公网 IP 访问，需额外加 `-m conntrack --ctstate DNAT` 的 hairpin MASQUERADE。
- **实机验证**：以跨网段 LXC 容器（源 `10.0.3.x`，回包必经宿主机）为测试客户端——`.12:2222` 可连、`.13:22`/`.13:80`/`.11:22` 全不可连（修复前这些由宿主机应答）、内网直连对照正常；宿主机 `ping` 公网 IP 由「能通」变为 100% 丢包；客户机出网 SNAT 会话 ESTABLISHED；5 个独立外部节点连接 `.12:2222` 全部成功。
- **缺陷③ 全端口透传抢先匹配，自定义端口映射被吃掉**：所有规则都以 `-I PREROUTING 1` 插入，后插入者在上；透传规则原本在端口映射之后插入，于是永远压在映射之上 —— 连 `公网IP:22002` 会先被透传改写成 `客户机:22002`，`22002 → 2222` 这条映射根本没机会生效。修复：把 `ensureIndependentIPv4Ingress()` 的调用提到端口映射循环之前，使透传落在映射之下、映射优先。
- **缺陷④（本次自引入并当日修复）**：守卫规则清理逻辑用 `line[idx+len("--comment "):]` 取注释，把注释之后的 ` -j DROP` 一并吞掉，于是"保留集合"永远匹配不上、**刚插入的守卫在下一次清理时被自己删掉**（线上实测启动后守卫数归零）。修复：按空白截断只取注释 token 并容忍 `iptables-save` 的引号形式，保留集合直接比较生成的注释串；新增 `backend/internal/lxc/ipv6_guard_test.go` 覆盖两种注释格式与普通规则行。

### 6. 🔗 独立公网 IP 互连修复：发夹 NAT、宿主机反射与守卫收窄 (v1.20.13)
- **问题**：分配独立公网 IP 后，**同一网段内的机器与宿主机自身用公网 IP 访问实例都失败**；用户对照"以前 PVE 下独立 IP 之间可以互连"，要求排查。
- **根因（架构差异）**：PVE 把公网 IP **配置在客户机内部**，VM 间二层直连、回包源即公网 IP，天然互通；CLICD 是**公网 IP 建在宿主机 + DNAT**：A 访问 `公网IP` 的包被改写目标为 B，B 回包时发现 A 与自己同网段 → 直接二层回给 A、绕过宿主机 → A 收到"来自内网地址的回复"而丢弃（conntrack 长期 `[UNREPLIED]`）。
- **修复① 发夹 NAT**：`EnsurePublicIPv4HairpinMasquerade()` 对「源与目标同网桥子网 + 该连接已被 DNAT」的流量 MASQUERADE（`-m conntrack --ctstate DNAT`），强制回包经宿主机；KVM/LXC 各一条，随时间启动与分配变更重新下发。**只影响经公网 IP 绕回的连接**，客户机直连与他人出网均不受影响。
- **修复② 宿主机反射**：新增 nat **OUTPUT** 反射 DNAT（全端口透传 + 逐条端口映射各一份，注释带 `-out` 后缀，随 `CleanPortMappings` 清理），使宿主机自己也能用公网 IP 访问实例。
- **修复③ 守卫收窄**：v1.20.12 的 `INPUT -d <ip> -j DROP` 过宽，把宿主机自身反射连接的回包也丢了（实测 conntrack 已 `SYN_RECV`、客户机回了 SYN-ACK 但母鸡收不到）。改为 **`-m conntrack --ctstate NEW`**：只拦新建入站连接，已建立会话放行；防护效果不变。守卫升级时按注释先删旧形态再插新形态（`deleteTaggedIPTablesRules`）。
...
- **双重控制机制**：
  ...
  - **交互菜单控制**：在 `sudo clicd` 交互菜单�� `9` 即可一键切换状态。
...
- **实机验证与多行脚本执行优化（测试机）**：
  - **排查修复多行执行异常**：此前若用户多行命令包含 `&&` 换行拼接（如同时点击了工具包和 Docker 安装），直接作为 `sh -c` 字符串执行时触发了 `sh: Syntax error: "&&" unexpected` 语法错误。
...
- **后端兜底截断 (`backend/internal/api/runtime.go`)**：
  ...
  - 超时时立即返回包含 `timed_out: true` 的基础兜底指标结构，彻底避免阻塞 HTTP 连接。
- **前端异步解耦 (`api.ts` & `Containers.tsx`)**：
  - `getContainerUsage` 显式设置 Axios 请求 `timeout: 5000`。
  - `fetchData` 将容器主列表数据与实时指标刷新完全解耦为静默异步执行（`refreshUsage(nextContainers).catch(() => {})`），列表秒开展示。
