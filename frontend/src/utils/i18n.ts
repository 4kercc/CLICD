const exact: Record<string, string> = {
  ...
  '输入错误': 'Input error',
  '双击调整 SSH/RDP 端口映射（公网入口 → 客户机真实端口）': 'Double-click to adjust the SSH/RDP port mapping (public entry → the real port inside the guest)',
  '调整服务端口映射': 'Adjust service port mapping',
  '没有可调整的端口映射': 'No adjustable port mapping',
  '该实例没有 SSH/RDP 端口映射，无法调整入口指向的端口。': 'This instance has no SSH/RDP port mapping, so the entry port cannot be adjusted.',
  '客户机真实端口': 'Real port inside the guest',
  '例如客户机 sshd 实际监听 2222 而不是 22，就填 2222 —— 之后用公网地址的这个入口即可连上。请确认客户机上的服务真的监听该端口，否则入口会不可达。': 'For example, if the guest sshd actually listens on 2222 instead of 22, enter 2222 — the public entry then reaches it. Make sure the service inside the guest really listens on that port, otherwise the entry stays unreachable.',
  '端口映射已更新': 'Port mapping updated',
  '公网入口现在转发到客户机端口 2222，请确认客户机上的服务确实监听该端口。': 'The public entry now forwards to guest port 2222. Make sure the service inside the guest really listens on that port.',
  '该实例已分配独立公网 IPv4': 'This instance has a dedicated public IPv4 assigned',
  ...
  '逻辑占用': 'Logical size',
  '实际独占': 'Actually freed',
  '快照文件自身的大小，包含与运行磁盘共享的块': 'Size of the snapshot file itself, including blocks still shared with the running disk',
  '删除该快照能真正释放的空间。若主机文件系统支持 reflink 写时复制，刚拍完的快照几乎全部与运行磁盘共享，独占值会远小于逻辑占用，并随着系统继续写入逐步变大': 'Space that deleting this snapshot would actually free. With reflink copy-on-write on the host filesystem a fresh snapshot shares almost everything with the running disk, so this is far smaller than the logical size and grows as the guest keeps writing',
  '该主机或该类型快照无法统计独占大小': 'Exclusive size cannot be measured on this host or for this snapshot type',
  '备注 (可选，便于日后回退时辨认版本)': 'Note (optional, so you can tell versions apart when restoring later)',
  '备注 (可选，便于日后回退时辨认版本)：': 'Note (optional, so you can tell versions apart when restoring later):',
  '例如：装完宝塔面板、升级内核前、部署上线前基线…': 'e.g. after installing aaPanel, before a kernel upgrade, pre-release baseline…',
  '例如：批量升级前基线、装完业务环境…': 'e.g. pre-upgrade baseline for the batch, business stack installed…',
  '留空则仅记录时间与创建者。': 'Leave empty to record only the time and creator.',
  '该备注会写入本批全部快照。': 'This note is stored on every snapshot in this batch.',
  '当前实例运行中，拍摄快照需要先关机，完成后会自动重启。': 'This instance is running. A snapshot requires shutting it down first; it restarts automatically afterwards.',
  ...
  '母盘': 'master',
  '仅控制': 'Only controls',
  // --- boot / extra ISO pickers ---
  '启动光盘覆盖 (留空为使用镜像自带光盘)': 'Boot disc override (leave empty to use the image default)',
  '使用镜像自带光盘（推荐）': "Use the image's own disc (recommended)",
  '覆盖后会替换虚拟机原本挂载的安装/引导光盘，一般不需要修改。': 'Overriding replaces the install/boot disc the image ships with; you normally do not need to change this.',
  '额外挂载光盘 (独立光驱，不影响启动光盘)': 'Extra disc (separate drive, does not affect the boot disc)',
  '挂到独立光驱（SATA sdc）：PE 启动盘可以继续从自带光盘启动 PE，进入 PE 后再从这块光驱安装系统。虚拟机运行中修改会即时热插拔。': 'Attached to a separate drive (SATA sdc): a PE image keeps booting PE from its own disc, and you can install the OS from this drive once inside PE. Changes apply instantly on a running VM.',
  '不挂载': 'Not mounted',
  '自定义路径…': 'Custom path…',
  '磁盘镜像': 'disk image',
  '硬件与引导配置已更新，额外光盘已即时挂载/弹出。': 'Hardware and boot settings updated; the extra disc was mounted/ejected immediately.',
